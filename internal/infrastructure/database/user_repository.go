...
func (r *UserRepository) Save(ctx context.Context, entity *user.User) error {
	query := `
		INSERT INTO users (id, name, email, phone, cpf, firebase_id, role, created_at, updated_at)
...
func (r *UserRepository) FindByID(ctx context.Context, id string) (*user.User, error) {
	query := `
		SELECT id, name, email, phone, cpf, firebase_id, role, created_at, updated_at
		FROM users
		WHERE id = $1
	`
...
func (r *UserRepository) FindCaregivers(ctx context.Context, elderlyID string) ([]*user.User, error) {
	query := `
		SELECT u.id, u.name, u.email, u.phone, u.cpf, u.firebase_id, u.role, u.created_at, u.updated_at
		FROM users u
